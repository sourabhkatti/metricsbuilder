...
class controller:
    ...
    ORGANIZATION_ID = ""
    TEAMSERVER_URL = ""
    API_KEY = "51YoF4w7Uaxrr8tcS3mKoMcJHkI054kF"
    SERVICE_KEY = "BU9KLQQOMA0N257P"
    USERNAME = ""
                ...
                for user in jsonreader["users"]:
                    try:
                        ...
                        if float(date_diff) > float(days):
                            linetowrite = (
                                "\t%d. %s has not logged in for %d days" % (usercount, user['user_uid'], date_diff))
                            filewriter.write(linetowrite + "\n")
                            print(linetowrite)
                            usercount += 1
            ...
            if jsonreader["success"] is True:
                ...
                for user in jsonreader["users"]:
                    try:
                        ...
                        earlier_date = (datetime.datetime.today() - datetime.timedelta(30)).strftime("%Y-%m-%d")
                ...
                print(str(login_percentage) + "% of users have logged into teamserver the past 30 days.\n")
                # linetowrite = (str(earlier_date) + "," + str(todaydate_formatted) + ',' + str(login_percentage) + ',')

            earlier_date = (datetime.datetime.today() - datetime.timedelta(800)).strftime("%Y-%m-%d")
...
controller.getUsersLoggedInDays(days=5)
...
controller.metricsbuilder(days=7)
# controller.test()
